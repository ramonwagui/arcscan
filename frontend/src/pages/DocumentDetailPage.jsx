...
import { documentsApi, categoriesApi, signaturesApi } from '../lib/api'
                ...
                <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 border-b border-slate-100 pb-8">
                    ...
                    <div className="flex items-center gap-3 lg:self-end">
                        <button
                            ...
                            className="btn-secondary py-3 px-6 h-12 bg-white text-primary-600 border-primary-100 hover:border-primary-500 hover:bg-primary-50"
                        >
                            <Shield size={18} />
                                                        ...
                                                        <div className="flex items-center gap-2 mt-1">
                                                            ...
                                                            {log.ip && (
                                                                <>
                                                                    ...
                                                                    <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest font-mono">IP: {log.ip}</span>
                                                                </>
                                                            )}
                                                        </div>
                                                    </div>
            ...
            {/* Signature Request Modal */}
            {
                isSignatureModalOpen && (
                    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm animate-fade-in">
                        <div className="bg-white rounded-[2.5rem] w-full max-w-md p-8 shadow-2xl border border-slate-100 animate-slide-up">
                            <div className="flex items-center justify-between mb-6">
                                <h3 className="text-2xl font-black text-slate-900 tracking-tight">Solicitar Assinatura</h3>
                                <button
                            ...
                            <p className="text-xs font-bold text-slate-500 mb-8 leading-relaxed">
                                Um link único e seguro será gerado e poderá ser enviado para esta pessoa assinar digitalmente através do nosso portal público.
                            </p>

                            <form onSubmit={async (e) => {
                                e.preventDefault();
                                if (!signerName || !signerEmail || isRequestingSignature) return;

                                setIsRequestingSignature(true);
                                try {
                                    const response = await signaturesApi.request(doc.id, [signerEmail]);
                                    const secureLink = `${window.location.origin}/sign/${response.token}`;
                                    navigator.clipboard.writeText(secureLink);
                                    toast.success('Link seguro gerado e copiado para a área de transferência!');
                                    ...
                                    const logs = await documentsApi.getAudit(doc.id);
                                    setAuditLogs(logs);
                                } catch (err) {
                                    console.error(err);
                                    toast.error('Erro ao gerar solicitação segura no servidor.');
                                } finally {
                                ...
                                <div>
                                    <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Nome do Signatário</label>
                                    <input
                                        type="text"
                                        required
                                        value={signerName}
                                ...
                                <div>
                                    <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">E-mail do Signatário</label>
                                    <input
                                        type="email"
                                        required
                                        value={signerEmail}
                                ...
                                <button
                                    type="submit"
                                    disabled={isRequestingSignature || !signerName || !signerEmail}
                                    className="btn-primary w-full h-14 mt-4"
                                >
                                    {isRequestingSignature ? <div className="w-5 h-5 spinner" /> : 'Gerar Link de Assinatura'}
                                </button>
                            </form>
                        </div>
                    </div>
